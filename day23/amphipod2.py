...
start_hallway = [EMPTY] * 11

# Rooms along hallway. At end, should be [[AAAA][BBBB][CCCC][DDDD]] (modified for Part 2)
# Introduce additional Part 2 lines here
start_rooms = [
    [EMPTY, D, D, EMPTY],
    [EMPTY, C, B, EMPTY],
    [EMPTY, B, A, EMPTY],
...
complete_rooms = [[A, A, A, A], [B, B, B, B], [C, C, C, C], [D, D, D, D]]
    ...
    for line in infile:
        ...
        if len(pods) > 0:
            for i, c in enumerate(pods):
                start_rooms[i][3 if room_line != 0 else 0] = charmap[c]
            room_line += 1
...
def expand(state: State)->List[State]:
    ...
    for i, room in enumerate(state.rooms):
        ...
        in_place = [r == reverse_roommap[i] for r in room]
...
while(len(open_list) > 0):
    ...
    if cur.rooms == complete_rooms:
        print(f"Part 2: Min Cost = {cur.cost}")
        break
